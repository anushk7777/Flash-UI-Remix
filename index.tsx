...
const COMPONENT_TYPES = ['UI Component', 'Button', 'Card', 'Form', 'Loader', 'Navigation', 'Widget'];
...
function App() {
  ...
  const [componentType, setComponentType] = useState<string>('UI Component');
...
\`{ "name": "Persona Name", "html": "..." }\`
  ...
  const handleSendMessage = useCallback(async (manualPrompt?: string) => {
    ...
    try {
        ...
        const stylePrompt = `
Generate 3 distinct, highly evocative design directions for a ${componentType} described as: "${trimmedInput}".
...
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions (e.g. ["Tactile Risograph Press", "Kinetic Silhouette Balance", "Primary Pigment Gridwork"]).
        ...
        const generateArtifact = async (artifact: Artifact, styleInstruction: string) => {
            try {
                const prompt = `
You are Flash UI. Create a stunning, high-fidelity ${componentType} for: "${trimmedInput}".
...
3. **Motion & Reactivity**: The ${componentType} MUST be highly reactive and dynamic. Include advanced CSS/JS interactions (hover, active, focus states, micro-interactions, spring physics, or magnetic effects).
...
Return ONLY RAW HTML. No markdown fences.
  ...
  return (
    <>
        <div className={`creator-credit ${hasStarted ? 'hide-on-mobile' : ''}`}>
            created by Anushk
        </div>
